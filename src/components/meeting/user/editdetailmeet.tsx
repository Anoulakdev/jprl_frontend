...
const EditForm = () => {
  ...
  const [meet, setMeet] = useState<DetailMeet | null>(null);
  ...
  useEffect(() => {
    if (id) {
      ...
      axiosInstance
        .get<DetailMeet>(`/detailmeets/${decryptedId}`)
        .then((response) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append("userCode", meet.userCode);
      formDataToSend.append("content", meet.content);

      // ถ้ามีการอัปโหลดใหม่ ใช้อันใหม่ ไม่งั้นใช้ชื่อเดิม
      if (uploadedImage) {
        formDataToSend.append("meetimg", uploadedImage); // รูปภาพแนบโดยตรง
      }

      await axiosInstance.put(`/detailmeets/${meet.id}`, formDataToSend, {
        headers: {
          ...
          {/* <div className="mb-4.5 flex flex-col gap-4.5 md:flex-row lg:flex-row">
            <div className="w-full md:w-1/4 lg:w-1/4">
              {act?.meetimg && (
                <img
                  className="mt-5"
                  src={`${process.env.NEXT_PUBLIC_API_URL}/upload/activity/${act.meetimg}`}
                  alt={act.meetimg}
                  width={200}
                  height={200}
                />
