...
const DetailMeetList: React.FC = () => {
  const [data, setData] = useState<DetailMeet[]>([]);
  const [filteredData, setFilteredData] = useState<DetailMeet[]>([]);
  const [filterText, setFilterText] = useState<string>("");
  const [loading, setLoading] = useState<boolean>(true);
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [rowsPerPage, setRowsPerPage] = useState<number>(10);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState<boolean>(false);
  const [detailmeetToDelete, setDetailMeetToDelete] =
    useState<DetailMeet | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await axiosInstance.get(`/detailmeets`);
  ...
  const handleEdit = (id: number) => {
    ...
    router.push(`/meeting/user/detail/edit/${encryptedId}`);
  };

  const handleDeleteClick = (id: number) => {
    const detailmeet = data.find((detailmeet) => detailmeet.id === id);
    if (detailmeet) {
      setDetailMeetToDelete(detailmeet);
      setIsDeleteModalOpen(true);
    }
  };

  const handleConfirmDelete = async () => {
    if (detailmeetToDelete) {
      try {
        await axiosInstance.delete(`/detailmeets/${detailmeetToDelete.id}`);
        const deletedData = data.filter(
          (detailmeet) => detailmeet.id !== detailmeetToDelete.id,
        );
        setData(deletedData);
  ...
  const handleFilter = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = data.filter(
      (meet) =>
        meet.content.toLowerCase().includes(value) ||
        new Date(meet.createdAt).toLocaleDateString("en-GB").includes(value),
    );
  ...
  const columns = [
    {
      name: "#",
      cell: (row: DetailMeet, index: number) => {
    ...
    {
      name: "ຫົວ​ຂໍ້ປະ​ຊຸມ",
      selector: (row: DetailMeet) => row.meeting?.name,
      sortable: true,
      width: "30%",
      cell: (row: DetailMeet) => (
    ...
    {
      name: "ເນື້ອ​ໃນ​ການ​ມາ​ປະ​ຊຸມ",
      selector: (row: DetailMeet) => row.content,
      sortable: true,
      width: "30%",
      cell: (row: DetailMeet) => (
        <div className="overflow-hidden whitespace-normal break-words">
    ...
    },
    {
      ...
      cell: (row: DetailMeet) => (
        <Image
          ...
          src={`${process.env.NEXT_PUBLIC_API_URL}/upload/meeting/${row.meetimg}`}
          width={500}
          height={500}
          alt={row.meetimg}
        />
    ...
    {
      name: "ສະ​ຖານ​ະ",
      selector: (row: DetailMeet) => row.approved,
      sortable: true,
      cell: (row: DetailMeet) => {
        let text = "";
        let colorClass = "";
    ...
    },
    {
      ...
      cell: (row: DetailMeet) => {
        ...
        const dateactive = moment(row.meeting.dateactive).format("YYYY-MM-DD");
