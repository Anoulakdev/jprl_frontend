  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `/reports/selectdaterangecount?datestart=${formattedStartDate}&dateend=${formattedEndDate}`,
      );
    ...
    if (reportData) {
      const worksheet = XLSX.utils.json_to_sheet(
        reportData.map((item, index) => ({
          "ລ/ດ": index + 1,
          "ວັນ​ທີ": moment(item.dateactive).format("DD/MM/YYYY"),
          "ຊື່​ກິດ​ຈະ​ກຳ": item.activityName,
          "ເພດ​ຊາຍ": item.Male,
          "ເພດ​ຍິງ": item.Female,
          ລວມ: item.total,
        })),
      ...
      saveAs(
        data,
        `ສັງ​ລວມຈຳ​ນວນ​​ເຂົ້າ​ຮ່ວມ​ກິດ​ຈະ​ກຳ​ຕາມ${DateStart}-${DateEnd}.xlsx`,
      );
    }
  };
          ...
          <table className="w-full table-auto">
            <thead>
              <tr className="bg-[#d9e0e9] text-left text-lg font-bold dark:bg-dark-2">
                <th className="min-w-[60px] px-2 py-4 font-medium text-dark dark:text-white xl:pl-7.5">
                  ລ/ດ
                </th>
                <th className="min-w-[110px] px-2 py-4 font-medium text-dark dark:text-white">
                  ວັນ​ທີ
                </th>
                <th className="min-w-[350px] px-2 py-4 font-medium text-dark dark:text-white">
                  ຊື່ກິດ​ຈະ​ກຳ
                </th>
                  ...
                  <tr key={index}>
                    ...
                    <td className="border-b border-[#eee] px-2 py-2 dark:border-dark-3">
                      <p className="text-dark dark:text-white">
                        {moment(item.dateactive).format("DD/MM/YYYY")}
                      </p>
                    </td>
                    <td className="border-b border-[#eee] px-2 py-2 dark:border-dark-3">
                      <p className="text-dark dark:text-white">
                    ...
                    <td className="border-b border-[#eee] px-2 py-2 dark:border-dark-3">
                      <p className="text-dark dark:text-white">{item.Male}</p>
                    </td>
                    <td className="border-b border-[#eee] px-2 py-2 dark:border-dark-3">
                      <p className="text-dark dark:text-white">{item.Female}</p>
                    </td>
                    <td className="border-b border-[#eee] px-2 py-2 dark:border-dark-3">
                      <p className="text-dark dark:text-white">{item.total}</p>
                    </td>
                  </tr>
                ))
              ) : (
            ...
            {reportData.length > 0 && (
              <tfoot>
                <tr className="bg-[#eef3f8] font-bold dark:bg-dark-2">
                  <td
                    className="px-2 py-3 text-center text-dark dark:text-white"
                    colSpan={3}
                  >
                    ລວມທັງໝົດ
                  </td>
                  <td className="px-2 py-3 text-dark dark:text-white">
                    {reportData.reduce(
                      (sum, item) => sum + Number(item.Male),
                      0,
                    )}
                  </td>
                  <td className="px-2 py-3 text-dark dark:text-white">
                    {reportData.reduce(
                      (sum, item) => sum + Number(item.Female),
                      0,
                    )}
                  </td>
                  <td className="px-2 py-3 text-dark dark:text-white">
