...
const Approved: React.FC = () => {
  ...
  const fetchData = async (meetingId: number) => {
    try {
      ...
      const response = await axiosInstance.get(
        `/detailmeets/listapproved?meetingId=${meetingId}&approved=2`,
      );
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((meet) => {
      const code = meet.userCode?.toLowerCase() || "";
      const firstname = meet.user?.firstname?.toLowerCase() || "";
      const tel = meet.user?.tel?.toLowerCase() || "";
      return (
  ...
  const columns: TableColumn<DetailMeet>[] = [
    {
      name: "#",
    ...
    {
      name: "ໜ່ວຍ",
      selector: (row) => row.user?.unit?.name,
      sortable: true,
      width: "240px",
    },
    {
