...
const UserList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await axiosInstance.get(`/users/superadmin`);
  ...
  const handleEdit = (id: number) => {
    ...
    router.push(`/user/superadmin/edit/${encryptedId}`);
  };
  ...
  const handleReset = async (id: number) => {
    ...
    if (confirm) {
      try {
        ...
        toast.success("ລີ​ເສັດລະ​ຫັດ​ສຳ​ເລັດ​ແລ້ວ");
      } catch (error) {
        console.error("Reset password failed:", error);
        toast.error("ລີ​ເສັດລະ​ຫັດ​ບໍ່ສຳ​ເລັດ");
      }
    }
  };
  ...
  const handleStatusChange = async (id: number, currentStatus: string) => {
    try {
      ...
      if (response.status === 200) {
        ...
        const updatedData = await axiosInstance.get(`/users/superadmin`);
